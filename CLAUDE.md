...
**その言葉の難しさではなく、学ぶ順番を表す。** 同じ束の中で、上から順に読める並びにする。
...
| 中級 | **その分野の入口。ここから始める** |
| 上級 | 中級を踏まえた先 |
| 特級 | 専門家の言葉。知らなくても困らない |

**土台を、その応用より上の段に置かない。** 半導体が上級でTSMCが中級、
指数関数的成長が特級でシンギュラリティが上級、という逆転を起こしていた。

対になる語は段を揃える（理事と監事、認証とOAuth）。
ひとつの束が1段に固まっていたら、たいてい振り直しが要る
（GitとGitHubは17本すべて上級だった）。
