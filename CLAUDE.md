...
**閉じる `**` の直前に、全角の閉じ括弧や句点を置かない。**

CommonMark では、閉じる `**` の直前が句読点や閉じ括弧で、かつ直後が文字だと、
閉じ記号として認められない。markedの不具合ではなく仕様どおりの動き。
日本語は括弧を強調の中に入れがちなので、繰り返し踏む。
...
直後が空白なら通る（`**2〜3週間でできます。** そのかわり` は問題ない）。
`npm run build` は、記号のまま残っていると**失敗する**（終了コード1）。
見た目がそのまま壊れるうえ、警告のままだと気づかずに公開してしまうため。
