...
- 難易度の目安: 初級=日常語（アプリ、AI）／中級=IT一般語（データベース、クラウド）／
  上級=開発者語（API、Git）／特級=専門家語（RLS）、**およびAIかけこみ寺が特に理解してほしい
  核心概念**（指数関数的成長、ゼロトラスト会計）。特級は「難しい」だけでなく「読破する価値が最も高い」の意味を持つ
...
**気にしなくてよい。** 書きたいまま `**…**` で囲めば、そのとおりに出る。

CommonMark には記号の両隣で開閉を決める規則があり、日本語の全角括弧や句点が
`**` に接すると開けない・閉じられないことがあった（`**「早さ」と「見えかた」**にあります` が通らない）。
そのたびに書きかたを歪めていたので、ビルドが `<strong>` へ変換する形にした（`toStrong`）。

コードの中（``` と backtick）は触らない。記号としてそのまま見せたいときは backtick で囲む。
対になっていない `**` が残っていると、ビルドは失敗する（終了コード1）。

## 例文の書きかた

**見出し語をそのまま使う。** その言葉を使った会話が見たくて例文を読むのに、
言い換えだけでは使いかたが分からない。

```
✗ 「なんでゲームの部品がAIに使われるの?」「単純な計算を一気にやる形が同じだったんだ」
✓ 「なんでゲームの部品がAIに使われるの?」「GPUのことだね。単純な計算を…」
```

`npm run build` は、例文に見出し語（title・yomi・english のいずれか）が
出てこないと失敗する。
