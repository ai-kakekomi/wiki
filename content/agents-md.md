...
AIツールに読ませる「決まりごと」のファイルの、会社をまたいだ共通の名前です。

## なぜ大事?

AIツールは会社ごとに違う名前の決まりごとファイルを読みます。**同じ中身をツールの数だけ書き直すのは無駄**なので、ひとつの名前に揃えようという動きが起きました。それが AGENTS.md です。
...
Claude Code は `AGENTS.md` を直接は読まず `CLAUDE.md` を読みますが、`CLAUDE.md` の中に「AGENTS.md を読んで」と1行書けば、同じ中身を両方のツールで使えます。名前が2つあるだけで、書くことは1つです。
