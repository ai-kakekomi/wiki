---
slug: github
title: GitHub
yomi: ギットハブ
english: GitHub
english_yomi: ギットハブ
proper_noun: true
difficulty: 中級
genres: [プログラミング]
related: [git, repository, pull-request, issue, microsoft]
sources:
  - title: GitHub Docs
    url: https://docs.github.com/ja
updated: 2026-09-05
---

## ひとことで

作ったプログラムを置いて、みんなで直していける場所です。

## なぜ大事?

世界中のソフトがここに置かれています。バイブコーディングで何か作ったとき、**手元のパソコンが壊れても残る場所**であり、他の人に見せたり手伝ってもらったりする窓口にもなります。

## やさしい解説

Gitという記録のツールを、インターネットの向こうに置いて共有できるようにしたサービスです。GitとGitHubは別物で、Gitがツール、GitHubが置き場だと思ってください。

置き場ひとつを「リポジトリ」と呼びます。中には、プログラムの本体、説明書（README）、直したい点のメモ（イシュー）が入ります。

無料で使えて、公開もできますし、自分だけ見える形にもできます。**AIかけこみ寺のOSSツールもここに置いてあります。**

はじめてなら、まず自分だけ見える置き場をひとつ作ってみるのがおすすめです。失敗しても誰にも見えません。

## 例文

- 「作ったアプリどこに置くの?」「GitHub。手元が壊れても残るからね」
- 「GitとGitHubって違うの?」「Gitがツールで、GitHubが置き場だよ」
