...
japanese: モデル文脈規約（AIと外のツールをつなぐ共通の差込口）
difficulty: 上級
genres: [AI, プログラミング]
...
AIと、外にあるツールやデータをつなぐための、共通の差込口の規格です。
...
AIも同じでした。カレンダーにつなぐ、地図につなぐ、社内のデータにつなぐ。相手ごとに専用の配線を書く必要があり、組み合わせの数だけ手間がかかっていました。MCPという形を決めておけば、ツールの側は1回作れば、どのAIからも使ってもらえます。
...
- 「便利そうだけど、勝手に予定を入れられたら困るな」「だから、どのツールを使わせるかは自分で選ぶ仕組みになってるんだ」
